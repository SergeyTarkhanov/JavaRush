...
public class Solution
{
    public static void main(String[] args) throws IOException
    {
        ...
        ArrayList<String> listParity = new ArrayList<String>();
        for (int i = 0; i < list.size(); i++)
        {

            if (list.get(i).length() %2 != 0)
                listParity.add(list.get(i) + " " + list.get(i) + " " + list.get(i));
            else
                listParity.add(list.get(i) + " " + list.get(i));
        }

        for (int i = 0; i < listParity.size(); i++)
        {
            System.out.println(listParity.get(i));
        }
    }
}
